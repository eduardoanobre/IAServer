...
public class IAServerClient {
	...
	public void init() {
		...
		log.info("[IA-SERVER-CLIENT] Initialized - Module: {}, Topic: {}, Base64: {}", moduleName, iaRequestsTopic,
				base64WrapperEnabled);
	}
	...
	/**
	 * Initialize KafkaTemplate with configuration
	 */
	...
	public boolean sendIaRequest(IaRequest request) {
		try {
			...
			if (log.isDebugEnabled()) {
				...
				Object contextShards = request.getOptions() != null ? request.getOptions().get(IaRequest.CONTEXT_SHARDS)
						: null;
	...
	public boolean sendConnectionTest() {
		try {
			...
			Map<String, Object> testMessage = Map.of("chatId", testChatIdPrefix + System.currentTimeMillis(), "source",
					"connection-test", "test", true, "timestamp", System.currentTimeMillis(), "module", moduleName);
			...
			log.error("[CONNECTION-TEST] EXCEPTION from module {}: {}", moduleName, e.getMessage(), e);
			return false;
		}
	}
	...
	public boolean sendStartupNotification() {
		try {
			...
			Map<String, Object> startupMessage = Map.of("source", moduleName + "-startup", "timestamp",
					System.currentTimeMillis(), "module", moduleName, "event", "MODULE_STARTUP");
			...
			log.error("[STARTUP-NOTIFICATION] EXCEPTION from module {}: {}", moduleName, e.getMessage(), e);
			return false;
		}
	}
	...
	private boolean sendMessage(String base64Message, String chatId) {
		try {
			...
			SendResult<String, String> result = kafkaTemplate.send(iaRequestsTopic, chatId, base64Message).get();
