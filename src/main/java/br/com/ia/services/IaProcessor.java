...
public class IaProcessor {
	...
	@PostConstruct
	public void init() {
		log.info("IaProcessor inicializado!");
	}

	@Bean
	public Function<Object, Object> processIa() { // NOSONAR
		log.info("Função processIa registrada!");

		return input -> {
			try {
				log.info("=== INICIO PROCESSAMENTO ===");
				log.info("INPUT recebido: {}", input);
				log.info("Tipo do input: {}", input.getClass().getSimpleName());
				
				// Verifica se é uma mensagem que deve ser ignorada
				if (input instanceof Map) {
					Map<String, Object> inputMap = (Map<String, Object>) input;
					
					// Se é uma mensagem de formato desconhecido, ignora silenciosamente
					if ("unknown_format".equals(inputMap.get("type"))) {
						log.info("IGNORANDO mensagem de formato desconhecido");
						return createIgnoredResponse("unknown_format");
					}
					
					// Se é mensagem de teste, ignora
					if (inputMap.containsKey("test") || inputMap.containsKey("source")) {
						log.info("IGNORANDO mensagem de teste");
						return createIgnoredResponse("test_message");
					}
					
					// Tenta converter para IaRequest
					try {
						IaRequest request = mapper.convertValue(inputMap, IaRequest.class);
						if (request.getChatId() != null) {
							log.info("PROCESSANDO IaRequest válido com chatId: {}", request.getChatId());
							...
							log.info("=== ENVIANDO RESPOSTA ===");
							return response;
						} else {
							log.warn("MAP sem chatId, ignorando");
							return createIgnoredResponse("no_chatId");
						}
					} catch (Exception e) {
						log.warn("Não foi possível converter Map para IaRequest: {}", e.getMessage());
						return createIgnoredResponse("conversion_failed");
					}
				}
				
				// Se for String JSON
				if (input instanceof String) {
					log.info("PROCESSANDO String JSON");
					try {
						Map<String, Object> parsed = mapper.readValue((String) input, Map.class);
						return processIa().apply(parsed); // Recursão controlada
					} catch (Exception e) {
						log.warn("Erro ao fazer parse do JSON: {}", e.getMessage());
						return createErrorResponse("invalid_json", e.getMessage());
					}
				}
				
				// Se for um objeto IaRequest direto
				if (input instanceof IaRequest) {
					log.info("PROCESSANDO IaRequest direto");
					IaRequest request = (IaRequest) input;
					log.info("ChatId: {}", request.getChatId());
					IaResponse response = processIaRequest(request);
					log.info("=== RESPOSTA DIRETA GERADA ===");
					log.info("Response: {}", response);
					return response;
				}
				
				log.warn("TIPO DE INPUT NÃO RECONHECIDO: {}", input.getClass());
				return createIgnoredResponse("unsupported_type");
				
			} catch (Exception e) {
				log.error("ERRO FATAL ao processar mensagem: {}", e.getMessage(), e);
				return createErrorResponse("processing_failed", e.getMessage());
			}
		};
	}

	private Map<String, Object> createIgnoredResponse(String reason) {
		return Map.of(
			"status", "ignored", 
			"reason", reason,
	...
	private Map<String, Object> createErrorResponse(String reason, String message) {
		return Map.of(
			"status", "error", 
			"reason", reason,
			"message", message,
	...
	// MÉTODO SEPARADO: processa IaRequest válido
	private IaResponse processIaRequest(IaRequest request) {
		// ==== chatId (único identificador do fluxo) ====
		String chatId = request.getChatId();
		if (chatId == null) {
			throw new IllegalArgumentException("chatId ausente no request.");
		}

		try {
			log.info("IAServer processando: {}", chatId);
			...
			if (apiKey == null)
				throw new IllegalArgumentException("api_key ausente nas opções.");
			...
			Integer topLogprobs = opts.containsKey("top_logprobs")
					? Integer.valueOf(String.valueOf(opts.get("top_logprobs")))
					: null;
			...
			Boolean background = opts.containsKey("background")
					? Boolean.valueOf(String.valueOf(opts.get("background")))
					: null;
			Boolean stream = opts.containsKey("stream") ? Boolean.valueOf(String.valueOf(opts.get("stream")))
					: null;
			...
			Integer maxToolCalls = opts.containsKey("max_tool_calls")
					? Integer.valueOf(String.valueOf(opts.get("max_tool_calls")))
					: null;
			Boolean parallelToolCalls = opts.containsKey("parallel_tool_calls")
					? Boolean.valueOf(String.valueOf(opts.get("parallel_tool_calls")))
					: null;
			...
			contextBlocks.add(
					ResponsesRequest.ContentBlock.builder().type("input_text").text(request.getPrompt()).build());
			...
			var builder = ResponsesRequest.builder().model(model).instructions(instructions).input(input)
					...
					.promptCacheKey(promptCacheKey).safetyIdentifier(chatId).store(store).background(background)
					.stream(stream).topP(topP).topLogprobs(topLogprobs).maxToolCalls(maxToolCalls)
					.parallelToolCalls(parallelToolCalls).previousResponseId(previousResponseId);
			...
			if (toolsRaw != null) {
				...
				if (toolsRaw instanceof List<?>) {
					tools = mapper.convertValue(toolsRaw,
							new TypeReference<List<ResponsesRequest.ToolDefinition>>() {
							});
				} else if (toolsRaw instanceof Map<?, ?>) {
					ResponsesRequest.ToolDefinition one = mapper.convertValue(toolsRaw,
							ResponsesRequest.ToolDefinition.class);
					tools = List.of(one);
				} else {
			...
			log.info("IAServer enviando response: {}", chatId);
			log.info("Response payload: {}", iaResponse);
			return iaResponse; // ← Retorno direto, sem MessageBuilder

		} catch (Exception e) {
			log.error("Erro no IAServer {}: {}", chatId, e.getMessage());
			...
			return IaResponse.error(chatId, "Erro ao processar IA: " + e.getMessage());
		}
	}
	...
	/**
	 * Constrói blocos de conteúdo a partir de context_shards em opts. Espera-se uma
	 * lista com elementos {type, version, stable, payload}. Ordena: estáveis
	 * primeiro, depois por type e version.
	 */
	private List<ResponsesRequest.ContentBlock> buildBlocksFromContextShards(Map<String, Object> opts) {
		...
		shardList.sort(Comparator.<Map<String, Object>, Boolean>comparing(s -> !Boolean.TRUE.equals(s.get("stable"))) // false<true
																														// =>
																														// stable
																														// primeiro
	...
	/**
	 * Heurística simples para distinguir falhas transitórias (reprocessar) de
	 * falhas permanentes. Ajuste conforme seu cliente/provedor real.
	 */
	...
	private static Double normalizeTopP(Map<String, Object> opts) {
		if (opts == null || !opts.containsKey(TOP_P) || opts.get(TOP_P) == null)
			return null;
		double v;
		try {
			v = Double.parseDouble(String.valueOf(opts.get(TOP_P)).trim());
		} catch (Exception e) {
			return null;
		}
		if (!Double.isFinite(v))
